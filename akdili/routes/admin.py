...
async def admin_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    # عد الطرود لكل تاجر بـ SQL مباشرة (بدون lazy loading)
    parcel_counts_raw = db.query(
        Parcel.merchant_id,
        sqlfunc.count(Parcel.id).label("cnt")
    ).group_by(Parcel.merchant_id).all()
    parcel_counts = {row.merchant_id: row.cnt for row in parcel_counts_raw}
